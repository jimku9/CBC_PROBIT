...
set.seed(2025)
data.dir     = "/Users/yu-cheng/Desktop/Yucheng/ML_CBC-PROBIT/"
all.dir      = "/Users/yu-cheng/Desktop/Yucheng/ML_CBC-PROBIT/"
function.dir = "/Users/yu-cheng/Desktop/Yucheng/ML_CBC-PROBIT/"
HB           = "HB_ALL.R"
...
save.image(paste0(all.dir, "probit_result_everything.RData"))
...
#########################
# Calculate UD and LARD #
#########################

alpha  = output$keep.Z_base - output$keep.V_choi
lambda = dnorm(alpha)/(1-pnorm(alpha))
delta  = lambda * (lambda - alpha)				
stdev  = sqrt(1 - delta)	
std.abs.dev = abs(output$keep.Z_choi - output$keep.V_choi)/stdev

# UD
UD.rank = NULL
UD.raw      = t(apply(std.abs.dev, 2:3, mean))
UD.summary  = apply(apply(UD.raw, 1, rank), 1, mean)
UD.rank     = cbind(UD.rank, rank(UD.summary))

# LARD
LARD.rank    = NULL
LARD.raw     = t(apply(std.abs.dev, 2:3, max))
LARD.summary = apply(apply(LARD.raw, 1, rank), 1, mean)
LARD.rank    = cbind(LARD.rank, rank(LARD.summary))

write.csv(UD.rank, paste(all.dir, "UD_rank.csv",  sep = ""), row.names=F)
write.csv(LARD.rank, paste(all.dir, "LARD_rank.csv",  sep = ""), row.names=F)
